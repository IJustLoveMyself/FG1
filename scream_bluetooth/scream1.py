...
def data_recev(port0,port1,lock,pipe0):
	...
	for dev in devices:
			...
			if len(info)>2:	
				...
				if info[2][2]=="SCREAM":
					print dev.addr 
					conn = Peripheral(dev.addr)	
	fd0=open("/dev/myuart0","r+")
	fd1=open("/dev/myuart1","r+")
	#fdm=open("motor.txt","a+")
...
#					port1.write(send_buf1) 
		...
		if(flag == 0x11):
					...
					if times==20:
						...
						times=0					
#					fds.write(data_buf)
		if(flag==0x11 or flag==0x10):
			ble_buf=send_buf1
			x=ble_buf.split(',')
			ble_send=","+x[1]+",0,"+x[3]+",0,"+x[5]+",0,"+x[7]+",0,"
			ble_buf="start"+ble_send+"stop"+"\r\n"
...
#					while True:
#						ble_send=pipe.recv()
#						conn.writeCharacteristic(0x0036,ble_send,withResponse=True)
