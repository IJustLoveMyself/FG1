...
##################扫描附近wifi，ssid和inform.txt中的ssid匹配，如果出现尝试连接，这里的扫描用的是pywifi自带的方法#########################################
	def wifi_scan_pywifi(self,iface): 
		while True:
			bsses = iface.scan_results()
			if bsses !=1 :
				break
...
######################在本机AP模式下与手机进行连接通信，获取信息########################
	def ap_trans_connect(self,iface):
		print "ap_trans_connect start"
		while True:
			pid=self.AP_mode()
			print "pid=",pid
			time.sleep(5)
			ipadress=self.get_ipadress()
			buf=self.get_inform(ipadress)
			if len(buf)==4 and buf.find("exit")!=-1:
				break
			sub=subprocess.Popen("sudo kill -9 "+pid,shell=True)
			inform=buf.split(":")
			while True:
				if self.wifi_connect(iface,inform[0],inform[1]) == 0:
					break
			print "wifi status is ",iface.status()
			if iface.status()==const.IFACE_CONNECTED:
				"wifi is connect"
				self.save_inform(buf)
				break
		print "break out ap_trans"
#########################wifi 连接#######################################       
	def wifi_connect(self,iface,ssid,password):
